...
const MappingStep = ({ headers, systemColumns, mappings, onMappingChange, projectName }: any) => {
    ...
    return (
        <ScrollArea className="h-96">
            ...
            <Table>
                <TableHeader><TableRow><TableHead>Sua Coluna (do CSV)</TableHead><TableHead>Nosso Campo no Sistema</TableHead></TableRow></TableHeader>
                <TableBody>
                    {headers.map((header: string) => (
                        <TableRow key={header}>
                            <TableCell className="font-medium">{header}</TableCell>
                            <TableCell>
                                <Select value={mappings[header] || 'ignore'} onValueChange={(value) => onMappingChange(header, value)}>
                                    <SelectTrigger><SelectValue placeholder="Selecione..." /></SelectTrigger>
                                    <SelectContent>{targetOptions.map((option: any) => (<SelectItem key={option.id} value={option.id}>{option.name}</SelectItem>))}</SelectContent>
                                </Select>
                            </TableCell>
                        </TableRow>
                    ))}
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent><DialogHeader><DialogTitle>Criar Nova Coluna</DialogTitle></DialogHeader>
                ...
                <DialogFooter><Button onClick={handleCreate} disabled={!name}>Criar Coluna</Button></DialogFooter>
            </DialogContent>
        </Dialog>
    );
};
...
export default function ImportProjectModal({ isOpen, onOpenChange, projectId, onImportSuccess }: ImportTasksModalProps) {
  ...
  useEffect(() => { if (isOpen) { setStep('upload'); setFile(null); setFilePath(''); setCsvHeaders([]); setCsvData([]); setMappings({}); setIsProcessing(false); } }, [isOpen]);
  ...
  const handleMappingChange = (csvHeader: string, systemColumnId: string) => {
    if (systemColumnId === 'new_column') setNewColumnModal({ open: true, csvHeader });
    else setMappings(prev => ({ ...prev, [csvHeader]: systemColumnId }));
  };
